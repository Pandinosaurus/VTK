...
void AttachPointFlagsArray(vtkUniformGrid* grid, const int flag, const std::string& label)
{
...
// the points by color -- 0(off) or 1(on) -- to indicate whether or not a
// particular flag is "ON"
void AttachCellFlagsArray(vtkUniformGrid* grid, const int flag, const std::string& label)
{
  assert("pre: grid should not be nullptr" && (grid != nullptr));
...
void AttachNodeAndCellGhostFlags(vtkMultiBlockDataSet* mbds)
{
  ...
  {
    vtkUniformGrid* myGrid = vtkUniformGrid::SafeDownCast(mbds->GetBlock(block));
    if (myGrid != nullptr)
    {
      // original IGNORE
      AttachPointFlagsArray(myGrid, vtkDataSetAttributes::DUPLICATEPOINT, "DUPLICATEPOINT");
      // original DUPLICATE
      AttachCellFlagsArray(myGrid, vtkDataSetAttributes::DUPLICATECELL, "DUPLICATECELL");
    }
  } // END for all blocks
}
...
vtkMultiBlockDataSet* GetDataSet(const int dimension, const int numPartitions, const int numGhosts)
{
  int wholeExtent[6];
  int dims[3];
...
//------------------------------------------------------------------------------
void RegisterGrids(vtkMultiBlockDataSet* mbds, vtkStructuredGridConnectivity* connectivity)
{
  assert("pre: Multi-block is nullptr!" && (mbds != nullptr));
...
vtkMultiBlockDataSet* GetGhostedDataSet(
  vtkMultiBlockDataSet* mbds, vtkStructuredGridConnectivity* SGC, int numGhosts)
{
...
int TestStructuredGridConnectivity_internal(int argc, char* argv[])
{
  ...
  for (int i = 0; i < 1; ++i)
  {
    for (int j = 0; j < 1; ++j)
    {
      ...
      vtkMultiBlockDataSet* mbds = GetDataSet(3, numberOfPartitions[i], numGhostLayers[j]);
      assert("pre: multi-block is nullptr" && (mbds != nullptr));
      ...
      int ext[6];
      mbds->GetInformation()->Get(vtkStreamingDemandDrivenPipeline::WHOLE_EXTENT(), ext);
...
bool CompareFieldsForGrid(vtkUniformGrid* grid)
{
  ...
  vtkDoubleArray* computedCellData =
    vtkArrayDownCast<vtkDoubleArray>(grid->GetCellData()->GetArray("COMPUTED-CellXYZ"));
  assert("pre: computedCellData is nullptr" && (computedCellData != nullptr));

  vtkDoubleArray* expectedCellData =
    vtkArrayDownCast<vtkDoubleArray>(grid->GetCellData()->GetArray("EXPECTED-CellXYZ"));
  ...
  vtkDoubleArray* computedPointData =
    vtkArrayDownCast<vtkDoubleArray>(grid->GetPointData()->GetArray("COMPUTED-NodeXYZ"));
  assert("pre: computePointData is nullptr" && (computedPointData != nullptr));

  vtkDoubleArray* expectedPointData =
    vtkArrayDownCast<vtkDoubleArray>(grid->GetPointData()->GetArray("EXPECTED-NodeXYZ"));
...
int SimpleTest(int argc, char** argv)
{
  ...
  int wholeExt[6];
  mbds->GetInformation()->Get(vtkStreamingDemandDrivenPipeline::WHOLE_EXTENT(), wholeExt);
